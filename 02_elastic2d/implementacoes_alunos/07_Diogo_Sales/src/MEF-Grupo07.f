...
c       isai  = sistema de saida das tensoes/deslocamentos:
c                 1 = cartesiano  (sx, sy, txy, sz ; u_x, u_y)
c                 2 = polar       (sr, sth, trt, sz ; u_r, u_th)
c               No modo polar, o centro e sempre a origem (0,0).
c
      read(nin,*) nnode,numel,numat,nen,ndf,ndm,isai
      xc0 = 0.d0
      yc0 = 0.d0
      if (isai .ne. 1 .and. isai .ne. 2) then
        print*, '*** ERRO: isai deve ser 1 (cartesiano) ou 2 (polar).'
        stop
      endif
c
...
c       i20,i21,i22 = 3 componentes no plano (cartesiano ou polar,
c                     conforme isai); i23 = sigma_z (axial)
c
c      i20    i21    i22     i23 
c      ------------------------------
c      |  sx  |  sy  |  txy  |  sz  |
...
c
     ...
     .           ndf,nout)
c
c     Calculo e impressao das tensoes por elemento:
c
      call tensao(a,m(i1),m(i2),m(i3),a(i4),a(i5),a(i6),a(i20),
     .            a(i21),a(i22),a(i23),isai,xc0,yc0,
     .            nnode,numel,ndm,ndf,nen,nout)
c
...
c
     ...
     .          a(i23),isai,xc0,yc0,nnode,numel,ndm,nen,ndf,nvtk)
      close(nvtk)
...
c ======================================================================
      subroutine tensao(e,ie,ix,id,x,f,u,c1a,c2a,c3a,sza,isai,xc0,yc0,
     .                  nnode,numel,ndm,ndf,nen,nout)
...
c
c     Saida conforme isai:
c       isai=1 (cartesiano): c1a=sx, c2a=sy, c3a=txy
c       isai=2 (polar)     : c1a=sr, c2a=sth, c3a=trt, obtidas
c                            rotacionando o tensor pelo angulo polar do
c                            centroide medido a partir de (xc0,yc0).
c     sza = sigma_z em ambos os casos:
c            EPD (iel=1,3): sz = nu*(sx+sy);  EPT (iel=2,4): sz = 0.
c ======================================================================
      integer nnode,numel,ndm,ndf,nen,nout,isai
      integer ie(*),ix(nen+1,*),id(ndf,*)
      real*8  e(10,*),x(ndm,*),f(ndf,*),u(*),xc0,yc0
      real*8  c1a(*),c2a(*),c3a(*),sza(*)
      integer nel,ma,iel,i,no,k,nenel,ii
      ...
      real*8  sx,sy,txy,sz,xc,yc,theta,cth,sth,sr,sth2,trt
      real*8  c1,c2,c3
c
      if (isai .eq. 2) then
        write(nout,'(a)')
     .    'Tensoes por elemento (nel, sr, sth, trt, sz):'
      else
        write(nout,'(a)')
     .    'Tensoes por elemento (nel, sx, sy, txy, sz):'
      endif
      do 500 nel = 1, numel
        ma  = ix(nen+1,nel)
        iel = ie(ma)
...
c
c       Tensoes no plano (cartesiano): sigma = D * eps
c
        sx  = d11*epsx + d12*epsy
        sy  = d12*epsx + d22*epsy
        txy = d33*gamxy
c
c       Tensao axial sigma_z:
c         EPD (iel=1,3): sz = nu*(sx+sy), pois eps_z = 0
c         EPT (iel=2,4): sz = 0 (superficie livre)
c
        if (iel .eq. 1 .or. iel .eq. 3) then
          sz = nu*(sx + sy)
        else
...
c
c       Componentes de saida conforme isai:
c
        if (isai .eq. 2) then
c
c         Polar: rotaciona (sx,sy,txy) pelo angulo do centroide,
c         medido a partir de (xc0,yc0). N=radial, T=circunferencial.
c
          xc = 0.d0
          yc = 0.d0
          do 300 ii = 1, nenel
            ...
            yc = yc + x(2,ix(ii,nel))
  300     continue
          xc = xc / dble(nenel) - xc0
          yc = yc / dble(nenel) - yc0
          theta = datan2(yc, xc)
          cth   = dcos(theta)
          sth   = dsin(theta)
          sr    = sx*cth*cth + sy*sth*sth + 2.d0*txy*sth*cth
          sth2  = sx*sth*sth + sy*cth*cth - 2.d0*txy*sth*cth
          trt   = (sy-sx)*sth*cth + txy*(cth*cth - sth*sth)
          c1 = sr
          c2 = sth2
          c3 = trt
        else
...
c
        ...
        sza(nel) = sz
c
        write(nout,'(i8,4e15.5)') nel, c1, c2, c3, sz
  500 continue
      return
      end
...
c ======================================================================
      subroutine wvtk(ix,id,x,f,u,c1a,c2a,c3a,sza,isai,xc0,yc0,
...
c       - Conectividade dos elementos
c       - Dados por elemento (CELL_DATA), conforme isai:
c           isai=1: sigma_x, sigma_y, tau_xy, sigma_z
c           isai=2: sigma_r, sigma_theta, tau_rtheta, sigma_z
c       - Dados nodais (POINT_DATA): vetor de deslocamento e, conforme
c           isai: (u_x,u_y) ou (u_r,u_theta)
c ======================================================================
      integer nnode,numel,ndm,nen,ndf,ix(nen+1,*),id(ndf,*),nvtk,isai
      real*8  x(ndm,*),f(ndf,*),u(*),aux(3),xc0,yc0
      real*8  c1a(*),c2a(*),c3a(*),sza(*)
      real*8  theta,cth,sth,un,ut
      integer i,j,k
c
c     Cabecalho VTK:
c
...
c
c     Dados por elemento (tensoes) - nomes conforme isai:
c
      if (ndf .eq. 2) then
        write(nvtk,'(a,i8)') 'CELL_DATA ', numel
        if (isai .eq. 2) then
          write(nvtk,'(a)') 'SCALARS sigma_r float 1'
        else
          write(nvtk,'(a)') 'SCALARS sigma_x float 1'
        endif
        write(nvtk,'(a)') 'LOOKUP_TABLE default'
        do 600 i = 1, numel
          write(nvtk,'(1e16.8)') c1a(i)
  600   continue
        if (isai .eq. 2) then
          write(nvtk,'(a)') 'SCALARS sigma_theta float 1'
        else
          write(nvtk,'(a)') 'SCALARS sigma_y float 1'
        endif
        write(nvtk,'(a)') 'LOOKUP_TABLE default'
        do 610 i = 1, numel
          write(nvtk,'(1e16.8)') c2a(i)
  610   continue
        if (isai .eq. 2) then
          write(nvtk,'(a)') 'SCALARS tau_rtheta float 1'
        else
          write(nvtk,'(a)') 'SCALARS tau_xy float 1'
        endif
        write(nvtk,'(a)') 'LOOKUP_TABLE default'
        do 620 i = 1, numel
          write(nvtk,'(1e16.8)') c3a(i)
  620   continue
        write(nvtk,'(a)') 'SCALARS sigma_z float 1'
        write(nvtk,'(a)') 'LOOKUP_TABLE default'
        do 630 i = 1, numel
          write(nvtk,'(1e16.8)') sza(i)
  630   continue
      endif
      if (ndf .eq. 2) then
c
c     Dados nodais: vetor de deslocamento (sempre em x,y para warp)
c
  ...
  400    continue
c
c        Componentes escalares do deslocamento, conforme isai:
c
         if (isai .eq. 2) then
c          Polar: u_r (radial) e u_theta (circunferencial),
c          rotacionados pelo angulo do no medido de (xc0,yc0).
           write(nvtk,'(a)') 'SCALARS u_r float 1'
           write(nvtk,'(a)') 'LOOKUP_TABLE default'
           do 410 i = 1, nnode
             aux(1) = f(1,i)
             aux(2) = f(2,i)
             ...
             k = id(2,i)
             if (k .gt. 0) aux(2) = u(k)
             theta = datan2(x(2,i)-yc0, x(1,i)-xc0)
             cth   = dcos(theta)
             sth   = dsin(theta)
             un    =  aux(1)*cth + aux(2)*sth
             write(nvtk,'(1e16.8)') un
  410      continue
           write(nvtk,'(a)') 'SCALARS u_theta float 1'
           write(nvtk,'(a)') 'LOOKUP_TABLE default'
           do 420 i = 1, nnode
             aux(1) = f(1,i)
             aux(2) = f(2,i)
             ...
             cth   = dcos(theta)
             sth   = dsin(theta)
             ut    = -aux(1)*sth + aux(2)*cth
             write(nvtk,'(1e16.8)') ut
  420      continue
         else
c          Cartesiano: u_x e u_y.
           write(nvtk,'(a)') 'SCALARS u_x float 1'
           write(nvtk,'(a)') 'LOOKUP_TABLE default'
           do 430 i = 1, nnode
             aux(1) = f(1,i)
             k = id(1,i)
  ...
  430      continue
           write(nvtk,'(a)') 'SCALARS u_y float 1'
           write(nvtk,'(a)') 'LOOKUP_TABLE default'
           do 440 i = 1, nnode
             aux(2) = f(2,i)
             k = id(2,i)
             if (k .gt. 0) aux(2) = u(k)
             write(nvtk,'(1e16.8)') aux(2)
  440      continue
         endif
         return
